...
export class AcademicsService {
    ...
    async getProgramNames() {
        return await this.prismaService.program.findMany({
            select: {
                ...
                classes: {
                    select: {name: true, id: true, sections: {select: {name:true, id: true}}}
                }
            },
    ...
    async createTsm(payload: TsmDto) {
        ...
        if(exists) throw new ConflictException("Selected teacher is already mapped to the selected subject")
    ...
    async removeTsm(id: number) {
        return await this.prismaService.teacherSubjectMapping.delete({ where: { id } });
    }
    ...
    async createTcsm(payload: TcsmDto) {
        const exists = await this.prismaService.teacherClassSectionMapping.findFirst({
            where: {
                teacherId: Number(payload.teacherId),
                classId: Number(payload.classId),
        ...
        if(exists) throw new ConflictException("Selected teacher is already mapped to the selected class")
    ...
    async removeTcsm(id: number) {
        return await this.prismaService.teacherClassSectionMapping.delete({ where: { id } });
    }
