...
export class ExaminationService {
    ...
    private calculateGradeForIntermediate(percentage: number): { grade: string; gpa: number } {
    ...
    private calculateGradeForUndergraduate(percentage: number): { grade: string; gpa: number } {
        if (percentage >= 90) return { grade: 'A+', gpa: 4.0 };
    ...
    async generateResultsForExam(examId: number, classId?: number) {
        ...
        for (const studentId in studentMarksMap) {
            const { student, totalMarks, obtainedMarks } = studentMarksMap[studentId];
            const percentage = (obtainedMarks / totalMarks) * 100;

            // Calculate grade based on program duration
            // 4-year programs use GPA system, others use percentage-based grading
            const isFourYearProgram = exam.program.duration?.includes('4');
            const { grade, gpa } = isFourYearProgram
    ...
    async generatePositionsForExam(examId: number, classId?: number) {
        try {
            ...
            for (const clsId in resultsByClass) {
                const classResults = resultsByClass[clsId].sort((a, b) => b.percentage - a.percentage);
    ...
    updatePosition(id: string, dto: UpdatePositionDto) {
        return this.prisma.position.update({
            ...
            data: {
                ...
                obtainedMarks: dto.obtainedMarks ? Number(dto.obtainedMarks) : undefined,
