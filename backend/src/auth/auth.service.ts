...
export class AuthService {
    ...
    async generateTokens(payload: { id: number | string, email: string, role?: string, permissions: any }) {
        if(!payload.role){
    ...
    async refreshTokens(payload: { id: number | string, email: string, role?: string, permissions: any }) {
    ...
    async createSuperAdmin(payload: CreateAdminDto) {
        ...
        if (existingAdmin) {
            throw new HttpException("Super Admin with this email already exists", HttpStatus.BAD_REQUEST);
        }
        // // hash the password
