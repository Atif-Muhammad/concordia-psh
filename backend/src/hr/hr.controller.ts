...
export class HrController {
  ...
  @Get('staff-leave-balance/:staffId')
  async getStaffLeaveBalance(@Param('staffId') staffId: string) {
    return await this.hrService.getStaffLeaveBalance(Number(staffId));
  }
  ...
  async deleteStaffAttendanceRecord(
    ...
    return await this.hrService.deleteStaffAttendanceRecord(
      Number(staffId),
      date,
  ...
  async deleteAutoGeneratedAttendance(@Query('date') date: string) {
    const parsedDate = new Date(date);
    return await this.hrService.deleteAutoGeneratedAttendance(parsedDate);
  }

  @Delete('staff-attendance/by-date')
  async deleteAttendanceByDate(
    @Query('date') date: string,
    @Query('role') role?: 'teaching' | 'non-teaching' | 'all',
  ) {
    return await this.hrService.deleteAttendanceByDate(date, role || 'all');
  }
