...
export const getStudentFeeHistory = async (studentId, type = "") => {
  try {
    const response = await axios.get(
      `${base_url}/fee-management/challan/history?studentId=${studentId}${type ? `&type=${type}` : ""}`,
      { withCredentials: true }
    );
    return response.data;
  } catch (error) {
...
const normalizeStaffAttendanceRows = (rows, date) => {
  ...
  for (const r of mapped) {
    const sid = Number(r?.staff?.id ?? r?.staffId);
...
export const getExtraChallansDedicated = async (filters = {}) => {
  try {
    ...
    const url = `${base_url}/fee/challans/extra/list${params.toString() ? '?' + params.toString() : ''}`;
    const { data } = await axios.get(url, { withCredentials: true });
    return data;
  } catch (error) {
    const message = error.response?.data?.message || error.response?.data?.error || error.message || "Something went wrong";
