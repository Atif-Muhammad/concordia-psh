                        ...
                        <TableBody>
                          {isFetching ? (
                            ...
                            fetchedStudents.map(student => {
                              ...
                              const auditLines = [];
                              if (att?.generatedAt) auditLines.push(`Generated: ${new Date(att.generatedAt).toLocaleString()}`);
                              if (att?.markedAt && !att?.autoGenerated) auditLines.push(`Last marked: ${new Date(att.markedAt).toLocaleString()}`);
                              const auditTitle = auditLines.join('\n');

                              return <TableRow key={student.id} className={isLocked ? "opacity-80" : ""}>
                                ...
                                <TableCell className="py-2 px-3 text-sm">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    {!isLocked && auditTitle && (
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                          <span className="inline-flex items-center text-muted-foreground/50 cursor-help">
